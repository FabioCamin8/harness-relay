...
The initial verdict at `62bb05d` was superseded after cumulative Standards and Spec review found symlink-parent escape, cleanup of clean committed work, incomplete lifecycle-persistence coverage, and unbounded validation evidence. Those findings are corrected at code candidate `7afa87fa3629469acc3f6d7d63399813fffc18b8`. G09-G13 are `PASSED` on Debian GNU/Linux 13: 67/67 source tests passed, followed by 67/67 from the installed wheel outside the checkout on CPython 3.13.5 as UID 65534 and CPython 3.9.25.

The exact-candidate tests cover parent and final-component symlink rejection, collision/traversal rejection, preservation of dirty, unmerged, and clean committed work, tracked/untracked/ignored content plus `HEAD` integrity, owned process-group timeout/cancel, parent disconnect, interrupted terminal persistence without a stale running record, recursion refusal, strict MCP version negotiation and request IDs, enabled-only runtime-validated tools, responsive status/cancel, validation redaction/bounds, and inference-free doctor behavior. Symlink target content outside the worktree is explicitly outside the post-hoc integrity boundary; native sandbox enforcement remains required. Maintainer review of stacked PR [#4](https://github.com/FabioCamin8/harness-relay/pull/4) remains pending.

### PR-5 evidence

At code candidate `7afa87fa3629469acc3f6d7d63399813fffc18b8`, G14 is `PASSED`. The offline installed-artifact workflow covers setup preview/apply, repeat no-op, typed MCP delegation through a fake native worker, an isolated edit, independently executed validation, source preservation, master-model config independence, uninstall preservation, and retained worktree/result evidence. The exact wheel passed 67/67 as UID 65534 on CPython 3.13.5 and 67/67 on CPython 3.9.25, both outside the checkout without `PYTHONPATH`. Wheel SHA-256: `1e4223471fed072d2bbc5569e02afc1fc757cc10a50fc5a7429aefea92054b77`; sdist SHA-256: `2fe56c1dc3df1aed268cf277afe54e9c97ead47d4006ca418b2cdeb2cd33d1c6`. Both extracted artifacts passed the public-data scan.
